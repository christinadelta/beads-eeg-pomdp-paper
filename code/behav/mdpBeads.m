function ll = mdpBeads(params,R,sequences,choicedata)

% unpack the R structure
beta        = R.beta;
maxDraws    = 10;
Ntrials     = max(size(sequences));
...
for trial = 1:Ntrials
    ...
    % test R.subdraws and trial_draws.. they should be compatible  
    subdraws_trial  = R.subdraws(trial);
    ...
    % I need to chnage the choice porbability columns based on what the
    % majority and minority urns are 
    % if trial_urn == 0 
    %     choiceprobs(:,1) = trialChoices(:,2);
    %     choiceprobs(:,2) = trialChoices(:,1);
    %     choiceprobs(:,3) = trialChoices(:,3);
    % else
    %     choiceprobs = trialChoices;
    % end 
    ...
    for draw = 1 : nDraws
        ...
        prob                    = squeeze(cprob(trial, draw, :));

        % estimate ll at each draw
        % ll                      = ll - log(choiceprobs(draw,:)*prob);
    ...
    elseif lastRow(2) == 1
        ...
        error('Invalid final choice');  % shouldn’t happen with my data
    end
