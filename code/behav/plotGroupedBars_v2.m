...
hold on;
% Assign colors to each group automatically
for i = 1:nGroups
...
for i = 1:nGroups
    % x positions for the center of each bar in group i
...
if ~isempty(pairs_within_condition)
    sigstar(pairs_within_condition, p_values_within);
end
...
hold on;
% Use the same colors as for draws
for i = 1:nGroups
    barHandles(i).FaceColor = colors(i,:);
end
